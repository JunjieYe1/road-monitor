  ...
  <div class="defect-page">
    <header class="defect-header neu-card">
      ...
      <div class="header-right">
        ...
        <span class="status-badge" :class="currentStage.key">{{
          currentStage.label
        }}</span>
      </div>
    </header>
    ...
    >
      ...
      <div class="neu-card flow-card">
        ...
        <!-- 阶段操作按钮 -->
        <div
          v-if="selStage === activeStageIdx && selStage < stages.length - 1"
          class="stage-actions"
        >
        ...
        <div class="modal-box neu-card">
          ...
          <div class="modal-field">
            ...
            <input
              v-model="newFu.date"
              type="date"
              class="neu-inset modal-input"
            />
          </div>
          <div class="modal-field">
            <label>检查人员</label>
            <input
              v-model="newFu.inspector"
              placeholder="输入检查人姓名"
              class="neu-inset modal-input"
            />
          </div>
          <div class="modal-field">
            <label>检查结论</label>
            <textarea
              v-model="newFu.conclusion"
              placeholder="描述检查结果..."
              class="neu-inset modal-textarea"
              rows="3"
            ></textarea>
          </div>
          ...
          <div class="modal-btns">
            <button class="modal-cancel" @click="showFollowupModal = false">
              取消
            </button>
...
const { overflowY, overflowX, onEnter, onLeave, remeasure } =
  useAdaptiveVerticalScroll(defectBodyRef);

// TODO: API - GET /api/defect/:id
const alert =
  alertStore.alerts.find((a) => a.id === id) || alertStore.alerts[0];
const defect = ref({
...
const stages = ref([
  {
    ...
    label: "发现",
    data: {
      time: defect.value.foundTime,
      operator: "巡检员 王某",
      action: "日常巡检发现路面病害，GPS定位上报",
      note: "现场照片已上传，坐标已记录",
    },
  },
  {
    ...
    label: "待处理",
    data: {
      time: `2024-03-${String(id + 10).padStart(2, "0")} 11:00`,
      operator: "调度员 李某",
      action: `已派单至${defect.value.unit}`,
      note: "要求 5 个工作日内完成处理",
    },
  },
  {
    ...
    data:
      alert.status === "pending"
        ? null
        : {
            time: `2024-03-${String(id + 11).padStart(2, "0")} 08:30`,
            operator: "张工长",
            action: "已到场勘查，材料调配中，预计明日完工",
            note: null,
          },
  },
  {
    ...
    data:
      alert.status === "completed"
        ? {
            time: `2024-03-${String(id + 12).padStart(2, "0")} 16:00`,
            operator: "张工长",
            action: "路面修复完成，已拍照存档",
            note: "修复材料：SBS改性沥青",
          }
  ...
  },
  {
    ...
    data:
      alert.status === "completed"
        ? {
            time: `2024-03-${String(id + 14).padStart(2, "0")} 10:00`,
            operator: "复测员 赵某",
            action: "现场复测通过，工单归档",
            note: "复测结果达标",
          }
...
const currentStage = computed(
  () => stages.value[activeStageIdx.value] || stages.value[1],
);
...
// 后续检查
const followups = ref<
  { date: string; inspector: string; conclusion: string; ok: boolean }[]
>(
  alert.status === "completed"
    ? [
        {
          date: "2024-06-10",
          inspector: "王某",
          conclusion: "路面状态良好，无新增病害",
          ok: true,
        },
        {
          date: "2024-09-15",
          inspector: "李某",
          conclusion: "未见异常，养护工作到位",
          ok: true,
        },
      ]
    : [],
...
function submitFollowup() {
  if (!newFu.value.date || !newFu.value.inspector || !newFu.value.conclusion)
    return;
...
.modal-cancel {
  padding: 8px 20px;
  border-radius: 10px;
  border: 1px solid var(--neu-stroke-muted-strong);
  background: var(--bg-color);
  color: #8a9aac;
  cursor: pointer;
  font-size: 13px;
