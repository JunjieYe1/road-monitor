...
const entries = [
  ...
  },
  {
    ...
    label: "全景洞察",
    desc: "实时地图 · 数据看板 · AI对话",
    mode: "insight" as AgentMode,
    view: "map" as CanvasViewType,
  },
  {
    key: "report",
    icon: "📄",
  ...
  },
  {
    ...
    label: "病害工单",
    desc: "全生命周期工单流转管理",
    mode: "operations" as AgentMode,
    view: "workorder" as CanvasViewType,
  },
  {
    key: "risk",
    icon: "⚠️",
    label: "风险预测",
    desc: "路段塌陷概率AI预测分析",
    mode: "predict" as AgentMode,
    view: "risk" as CanvasViewType,
  },
  {
    key: "compliance",
    icon: "🏆",
    ...
    desc: "服务单位红黑榜量化评估",
    mode: "insight" as AgentMode,
    view: "compliance" as CanvasViewType,
  },
  {
    key: "assess",
    icon: "🔍",
...
function handleSearch() {
  ...
  if (q.includes("报告") || q.includes("生成")) {
    view = "report";
  } else if (q.includes("履约") || q.includes("红黑榜")) {
    view = "compliance";
  } else if (q.includes("工单")) {
    mode = "operations";
    view = "workorder";
  } else if (q.includes("计划")) {
    mode = "operations";
    view = "workorder";
  } else if (q.includes("评估") || q.includes("拍照")) {
    mode = "predict";
    view = "risk";
  } else if (q.includes("风险") || q.includes("预测")) {
    mode = "predict";
    view = "risk";
  }

  canvasStore.setAgentMode(mode);
  if (view !== "map" && !isViewOpenDisabled(view))
    canvasStore.pushTab({ type: view });
