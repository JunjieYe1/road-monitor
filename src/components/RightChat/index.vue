  ...
  <div class="right-panel-wrap neu-card">
    ...
    <div class="session-toolbar">
        ...
        <transition name="session-dd-fade">
          ...
          >
            ...
            >
              ...
              <span
                v-if="formatLocaleSessionSub(c.updatedAt)"
                class="session-dd-item-sub"
                >{{ formatLocaleSessionSub(c.updatedAt) }}</span
              >
            </button>
          </div>
        </transition>
      ...
      >
        ...
        >
          <path
            d="M3 6h18M8 6V5a1 1 0 011-1h6a1 1 0 011 1v1m3 0v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6h14zM10 11v6M14 11v6"
            stroke="currentColor"
            stroke-width="1.65"
            stroke-linecap="round"
            stroke-linejoin="round"
      ...
      <button
        ...
        :class="{ active: wide }"
        :title="
          wide ? '收起对话栏（恢复三栏）' : '加宽对话栏（隐藏左侧栏，约半屏）'
        "
        @click="wide = !wide"
      >
        <svg
      ...
      >
        ...
        <div class="bubble-wrap" :class="msg.role">
          ...
          <div v-else class="bubble" :class="msg.role">
            <!-- 关联点位上下文徽章 -->
            <div
              v-if="msg.alertContext"
              class="bubble-alert-ctx"
              :class="msg.alertContext.severity"
            >
              <span class="ctx-icon">📍</span>
              <SeverityBadge
                :level="msg.alertContext.severity"
                size="sm"
                class="ctx-sev-badge"
              />
              <span class="ctx-info"
                >{{ msg.alertContext.type }} ·
                {{ msg.alertContext.address }}</span
              >
            </div>
            ...
            <div
              ...
              v-html="bubbleHtml(msg.content, msg)"
            ></div>
            <!-- ActionCard 操作卡片 -->
            <div
              v-if="msg.actionCard"
              class="action-card"
              :class="{ disabled: isViewOpenDisabled(msg.actionCard.type) }"
              @click="openCanvas(msg.actionCard!)"
            >
    ...
    <div class="input-area">
      ...
      <transition name="badge-fade">
        <div v-if="chatStore.attachedWorkorder" class="alert-badge-row">
          <div class="alert-badge workorder-badge">
            <span class="badge-icon">🧾</span>
            <SeverityBadge
              :level="chatStore.attachedWorkorder.severity"
              size="sm"
            />
            <span class="badge-type">{{
              chatStore.attachedWorkorder.type
            }}</span>
            <span class="badge-addr"
              >#{{ chatStore.attachedWorkorder.id }} ·
              {{ chatStore.attachedWorkorder.road }}</span
            >
            <button
              class="badge-del"
              @click="chatStore.clearAttachedWorkorder()"
              title="移除关联工单"
            >
              ✕
            </button>
      ...
      <transition name="mention-pop">
        <div
          v-if="showMention"
          class="mention-panel neu-card u-scrollbar-hidden"
        >
...
} from "../../utils/chatChartOptions";
import * as echarts from "echarts";
import { formatLocaleSessionSub } from "../../utils/localeFormat";
...
function disposeChatChartsIn(root: HTMLElement | null) {
  root?.querySelectorAll(".chat-chart-host").forEach((node) => {
    echarts.getInstanceByDom(node as HTMLElement)?.dispose();
  });
}

function hydrateChatChartEmbeds() {
  const root = chatBody.value;
  if (!root) return;
  disposeChatChartsIn(root);
  root.querySelectorAll(".chat-chart-host").forEach((node) => {
    const el = node as HTMLElement;
    const raw = el.getAttribute("data-chart-opt");
    if (!raw) return;
    try {
      const payload = JSON.parse(decodeURIComponent(raw)) as ChatChartPayload;
      el.style.height = payload.type === "pie" ? "280px" : "260px";
      el.style.minWidth = "220px";
      const chart = echarts.init(el);
      chart.setOption(buildChatChartEchartsOption(payload));
    } catch {
      /* 流式未完成或损坏时跳过 */
    }
  });
...
function onChatChartsResize() {
  if (!chatBody.value) return;
  chatBody.value.querySelectorAll(".chat-chart-host").forEach((node) => {
    echarts.getInstanceByDom(node as HTMLElement)?.resize();
  });
}

onMounted(() => {
  document.addEventListener("click", onDocClickSessionDd);
  window.addEventListener("resize", onChatChartsResize);
  updateInputMultiline();
  nextTick(() => {
    hydrateChatChartEmbeds();
    setupChatViewportObserver();
  });
});
onUnmounted(() => {
  document.removeEventListener("click", onDocClickSessionDd);
  window.removeEventListener("resize", onChatChartsResize);
  teardownChatViewportObserver();
  disposeChatChartsIn(chatBody.value);
});
...
let chatViewportObserver: IntersectionObserver | null = null;

function teardownChatViewportObserver() {
  chatViewportObserver?.disconnect();
  chatViewportObserver = null;
}

function setupChatViewportObserver() {
  teardownChatViewportObserver();
  const root = chatBody.value;
  if (!root) return;
  chatViewportObserver = new IntersectionObserver(
    (entries) => {
      const updates = entries
        .map((e) => ({
          messageId: Number((e.target as HTMLElement).dataset.messageId),
          visible: e.isIntersecting,
        }))
        .filter((u) => Number.isFinite(u.messageId));
      if (updates.length) uiStore.patchAssistantVisibility(updates);
    },
    { root, threshold: 0.14, rootMargin: "0px" },
  );
  root.querySelectorAll(".msg-row[data-message-id]").forEach((el) => {
    chatViewportObserver!.observe(el);
  });
}

watch(
  () => chatStore.messages.map((m) => m.id),
  (ids, prevIds) => {
    if (prevIds?.length) {
      const next = new Set(ids);
      const removed = prevIds.filter((id) => !next.has(id));
      if (removed.length) uiStore.clearAssistantVisibilityForMessages(removed);
    }
  },
);

watch(
  () => [chatStore.messages.length, chatBody.value] as const,
  async () => {
    await nextTick();
    setupChatViewportObserver();
  },
...
watch(
  ...
  async (id) => {
    ...
    uiStore.clearScrollRequest();
  },
);

const mentionItemsSource = [
  {
    type: "report" as CanvasViewType,
    icon: "📋",
    label: "巡检报告",
    desc: "查看 / 生成道路巡检报告",
  },
  {
    type: "compliance" as CanvasViewType,
    icon: "🏆",
    label: "履约画像",
    desc: "服务单位合规评分与排名",
  },
  {
    type: "workorder" as CanvasViewType,
    icon: "📑",
    label: "病害工单",
    desc: "病害工单看板与详情",
  },
  {
    type: "plan" as CanvasViewType,
    icon: "📅",
    label: "巡检计划",
    desc: "AI 生成月度巡检计划",
  },
  {
    type: "risk" as CanvasViewType,
    icon: "⚡",
    label: "风险预测",
    desc: "道路塌陷风险预判",
  },
  {
    type: "assess" as CanvasViewType,
    icon: "📸",
    label: "现场评估",
    desc: "上传照片 AI 分析病害",
  },
  {
    type: "map" as CanvasViewType,
    icon: "🗺️",
    label: "全景地图",
    desc: "返回地图总览",
  },
];
const selectedMentions = ref<(typeof mentionItemsSource)[0][]>([]);
...
const canSend = computed(
  () =>
    (inputText.value.trim() || selectedMentions.value.length > 0) &&
    !chatStore.isLoading,
);
...
function insertMention(item: (typeof mentionItemsSource)[0]) {
...
function removeMention(item: (typeof mentionItemsSource)[0]) {
...
function openCanvas(card: ActionCard) {
  if (isViewOpenDisabled(card.type)) return;
  canvasStore.pushTab({ type: card.type, title: card.title });
}

function bubbleHtml(content: string, msg: ChatMessage): string {
  if (msg.role !== "assistant") return renderChatMarkdown(content);
  viewportChartSyncKey.value;
  return renderChatMarkdown(content, {
    chartCaptionContext: {
      messageId: msg.id,
      resolveGlobalIndex: (ord: number) =>
        uiStore.globalIndexFor(msg.id, ord),
    },
  });
}
...
.chat-body {
  ...
  background: linear-gradient(
    var(--neu-angle),
    var(--neu-groove-from) 0%,
    var(--neu-groove-to) 100%
  );
  border-radius: 14px;
  box-shadow: var(--neu-inset-groove);
}
.stream-reply-hint {
  display: flex;
  align-items: center;
...
.stream-reply-hint-text {
  line-height: 1.4;
  font-weight: 600;
}
.stream-reply-hint-dots {
  display: flex;
  align-items: center;
  gap: 4px;
  flex-shrink: 0;
...
.bubble.assistant {
  background: linear-gradient(
    var(--neu-angle),
    var(--neu-convex-from) 0%,
    var(--neu-convex-mid) 60%,
    var(--neu-convex-to) 100%
  );
  color: var(--genshin-blue-dark);
  box-shadow: var(--neu-extrude-md);
  border: 1px solid var(--neu-border-highlight);
...
.bubble.user {
  ...
  color: #fff;
  box-shadow:
    3px 3px 9px rgba(26, 58, 82, 0.45),
    -2px -2px 6px rgba(74, 141, 183, 0.3);
...
.md-body :deep(blockquote) {
  ...
  border-left: 3px solid rgba(74, 141, 183, 0.45);
  background: rgba(74, 141, 183, 0.06);
  color: #4a5a6c;
  font-size: 0.96em;
}
...
.md-body :deep(hr) {
  border: none;
  border-top: 1px solid rgba(74, 141, 183, 0.2);
  margin: 0.65em 0;
}
.md-body :deep(table) {
...
.md-body :deep(.chat-ref-item-sum::-webkit-details-marker) {
  display: none;
}
...
.md-body :deep(.chat-ref-sum-sep) {
  flex-shrink: 0;
  opacity: 0.45;
  font-weight: 500;
}
.md-body :deep(.chat-ref-sum-page) {
  flex-shrink: 0;
  font-weight: 700;
  color: #4a5a6c;
}
...
.md-body :deep(.chat-ref-item[open] .chat-ref-chev) {
  transform: rotate(180deg);
}
...
/* <custom-picture> / <custom-chart> / <custom-filecard> / <custom-chain> */
.md-body :deep(.chat-picture-wrap) {
  margin: 0.65em 0;
  border-radius: 12px;
  overflow: hidden;
  border: 1px solid rgba(74, 141, 183, 0.2);
...
.md-body :deep(.chat-chart-caption--linked) {
  color: var(--genshin-blue-dark);
  background: rgba(74, 141, 183, 0.1);
...
.md-body :deep(.chat-chart-host) {
  margin: 0.65em 0;
  width: 100%;
  min-height: 240px;
  border-radius: 12px;
  border: 1px solid rgba(74, 141, 183, 0.18);
  background: rgba(255, 255, 255, 0.45);
  box-shadow: var(--neu-inset-groove);
}
.md-body :deep(.chat-filecard) {
  display: flex;
  ...
  background: linear-gradient(
    135deg,
    rgba(74, 141, 183, 0.08),
    rgba(45, 90, 123, 0.04)
  );
  box-shadow: var(--neu-extrude-sm);
}
.md-body :deep(.chat-filecard-icon) {
  font-size: 1.35em;
  line-height: 1;
  flex-shrink: 0;
  margin-top: 1px;
}
.md-body :deep(.chat-filecard-main) {
  min-width: 0;
  flex: 1;
  display: flex;
  flex-direction: column;
...
.md-body :deep(.chat-filecard-row) {
  display: flex;
  flex-wrap: wrap;
  align-items: baseline;
  gap: 8px 10px;
}
.md-body :deep(.chat-filecard-name) {
  font-weight: 700;
  color: var(--genshin-blue-dark);
  word-break: break-word;
}
.md-body :deep(.chat-filecard-meta) {
  font-size: 0.88em;
  color: #6a7a8c;
  font-weight: 600;
}
.md-body :deep(.chat-filecard-actions) {
  font-size: 0.9em;
}
.md-body :deep(.chat-filecard-link) {
  font-weight: 600;
  color: var(--genshin-blue);
  text-decoration: none;
  border-bottom: 1px dashed rgba(74, 141, 183, 0.45);
}
.md-body :deep(.chat-filecard-link:hover) {
  color: var(--genshin-gold-dark);
}
.md-body :deep(.chat-filecard-mute) {
  font-size: 0.88em;
  color: #b05050;
}
.md-body :deep(.chat-chain) {
  margin: 0.65em 0;
...
.md-body :deep(.chat-chain-item) {
  position: relative;
  padding-left: 18px;
  padding-bottom: 10px;
  border-left: 2px solid rgba(74, 141, 183, 0.28);
  margin-left: 6px;
}
.md-body :deep(.chat-chain-item--last) {
  border-left-color: transparent;
  padding-bottom: 0;
}
.md-body :deep(.chat-chain-dot) {
  position: absolute;
  left: -5px;
  top: 4px;
  width: 8px;
...
.md-body :deep(.chat-chain-body) {
  margin-top: -2px;
}
.md-body :deep(.chat-chain-title) {
  font-weight: 700;
  font-size: 0.95em;
  color: var(--genshin-blue-dark);
  line-height: 1.4;
}
.md-body :deep(.chat-chain-row) {
  margin-top: 4px;
  font-size: 0.88em;
  color: #5a6570;
  line-height: 1.45;
...
.md-body :deep(.chat-chain-desc-link) {
  color: var(--genshin-blue);
  font-weight: 600;
  text-decoration: none;
  border-bottom: 1px dashed rgba(74, 141, 183, 0.45);
}
.md-body :deep(.chat-embed-block) {
  margin: 0.5em 0;
  padding: 8px 10px;
  border-radius: 10px;
  font-size: 0.88em;
}
.md-body :deep(.chat-embed-block--error) {
  border: 1px dashed rgba(224, 112, 112, 0.45);
  background: rgba(224, 112, 112, 0.06);
  color: #a05050;
}

.bubble.user .md-body :deep(h1),
.bubble.user .md-body :deep(h2),
.bubble.user .md-body :deep(h3) {
  color: rgba(255, 255, 255, 0.95);
}
.bubble.user .md-body :deep(blockquote) {
  border-left-color: rgba(255, 255, 255, 0.35);
  background: rgba(255, 255, 255, 0.1);
  color: rgba(255, 255, 255, 0.88);
}
.bubble.user .md-body :deep(code) {
...
.bubble.user .md-body :deep(a) {
  color: var(--genshin-gold-light);
}
.bubble.user .md-body :deep(strong) {
  color: var(--genshin-gold-light);
}
.bubble.user .md-body :deep(th),
.bubble.user .md-body :deep(td) {
...
.bubble.user .md-body :deep(.chat-ref-heading) {
  color: rgba(255, 255, 255, 0.95);
}
...
.bubble.user .md-body :deep(.chat-ref-item-sum) {
  color: rgba(255, 255, 255, 0.9);
}
.bubble.user .md-body :deep(.chat-ref-sum-name) {
  color: rgba(255, 255, 255, 0.95);
}
.bubble.user .md-body :deep(.chat-ref-sum-page) {
  color: rgba(255, 255, 255, 0.88);
}
...
.bubble.user .md-body :deep(.chat-ref-empty) {
...
.bubble.assistant .bubble-alert-ctx {
  background: rgba(74, 141, 183, 0.1);
...
.bubble.assistant .ctx-info {
  color: var(--genshin-blue-dark);
}
.bubble.user :deep(.ctx-sev-badge.sev-badge.high) {
  background: rgba(255, 255, 255, 0.25) !important;
  color: #ffd0d0 !important;
}
.bubble.user :deep(.ctx-sev-badge.sev-badge.medium) {
  background: rgba(255, 255, 255, 0.25) !important;
  color: #ffe0b0 !important;
}
.bubble.user :deep(.ctx-sev-badge.sev-badge.low) {
  background: rgba(255, 255, 255, 0.25) !important;
  color: #b0f0d8 !important;
}
...
.action-card:hover {
  background: rgba(74, 141, 183, 0.14);
  border-color: rgba(74, 141, 183, 0.38);
...
.bubble-dt {
  ...
  border-radius: 8px;
  background: rgba(74, 141, 183, 0.06);
  border: 1px dashed rgba(74, 141, 183, 0.2);
  line-height: 1.45;
}
.bubble.user .bubble-dt {
  background: rgba(255, 255, 255, 0.12);
  border-color: rgba(255, 255, 255, 0.2);
  color: rgba(255, 255, 255, 0.85);
}
...
.alert-badge {
  ...
  border: 1px solid rgba(45, 90, 123, 0.18);
  box-shadow:
