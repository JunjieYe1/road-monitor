<template>
  <div class="workorder-view">
    ...
    <div class="kanban-wrap u-scrollbar-hidden">
      <div v-for="col in columns" :key="col.key" class="kanban-col">
        <div class="col-header" :style="{ borderTopColor: col.color }">
          ...
          <span class="col-count" :style="{ background: col.color + '22', color: col.color }">{{ col.cards.length
          }}</span>
        </div>
        <AdaptiveScrollY class="col-body">
          <div v-for="card in col.cards" :key="card.id" class="wo-card neu-card-sm"
            :class="{ selected: selCard?.id === card.id }" @click="openCard(card)">
            <div class="card-top">
    ...
    <transition name="drawer-slide">
      <div v-if="selCard" ref="detailDrawerRef" class="detail-drawer neu-card u-scrollbar-hidden"
        :style="{ overflowY: drawerOverflowY, overflowX: drawerOverflowX }" @mouseenter="drawerScroll.onEnter"
        ...
        <div class="drawer-body">
          <div class="d-row">
            <span class="d-label">工单编号</span><span class="d-val">#{{ selCard.id }} · WO-2024-{{
              String(selCard.id).padStart(4, "0")
            }}</span>
          </div>
          <div class="d-row">
            <span class="d-label">病害类型</span><span class="d-val">{{ selCard.type }}</span>
          </div>
          ...
          <div class="d-row">
            <span class="d-label">所在路段</span><span class="d-val">{{ selCard.road }}</span>
          </div>
          <div class="d-row">
            <span class="d-label">负责单位</span><span class="d-val">{{ selCard.unit }}</span>
          </div>
          <div class="d-row">
            <span class="d-label">创建时间</span><span class="d-val">{{ selCard.date }}</span>
          </div>
          <div class="d-row">
...
const baseCards = [
  {
    ...
    unit: "上城区建设工程总公司",
    date: "2024-03-15",
    desc: "路面纵向空洞，长约12m，宽3cm，位于主车道，需立即处理。",
  },
  {
    id: 2,
    alertId: 2,
    ...
    unit: "杭州市政养护有限公司",
    date: "2024-03-16",
    desc: "深度约9cm空洞，面积0.6㎡，危及行车安全，正在施工中。",
  },
  {
    id: 3,
    alertId: 9,
  ...
  },
  {
    ...
    unit: "江干区路桥工程有限公司",
    date: "2024-02-28",
    desc: "重载严重疏松，深约3cm，路段长60m，已完成修复待复测。",
  },
  {
    id: 9,
    alertId: 5,
  ...
  },
  {
    ...
    unit: "西湖区道路工程公司",
    date: "2024-02-10",
    desc: "双向严重疏松，深度约2.5cm，路段长40m，已完成验收。",
  },
  {
    id: 11,
    alertId: 8,
