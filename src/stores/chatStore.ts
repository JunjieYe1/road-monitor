...
function mapApiMessage(m: Record<string, unknown>, idNum: number): ChatMessage {
  ...
  const time = formatLocaleTimeHm(ts ? new Date(ts) : new Date());
  return {
    id: idNum,
    role,
    content: msg?.content ?? "",
...
function applyUiContextFromHint(hint: UiContextHint | null) {
  if (!hint) return;
  const canvas = useCanvasStore();
  if (hint.agentMode) canvas.setAgentMode(hint.agentMode);
...
export const useChatStore = defineStore("chat", () => {
  ...
  const activeKbRange = computed(() =>
    activeKbSelections.value.length
      ? activeKbSelections.value.map((k) => k.label).join("、")
      : "",
  );
  ...
  function attachWorkorder(workorder: WorkorderContext) {
    attachedWorkorder.value = workorder;
  }
  ...
  function addKb(entry: KbSelection) {
    if (
      !activeKbSelections.value.some(
        (k) => k.id === entry.id || k.label === entry.label,
      )
    ) {
  ...
  function removeKb(id: string) {
    activeKbSelections.value = activeKbSelections.value.filter(
      (k) => k.id !== id,
    );
  }
  ...
  async function deleteActiveConversation() {
    ...
    if (activeConversationKey.value) {
      if (!confirm("确定删除该会话？服务器上该对话的全部消息将被删除。"))
        return;
      ...
      if (conversations.value.length)
        await selectConversation(conversations.value[0].key, false);
      else startNewChat(false);
    } else {
      if (
        !confirm("当前为新会话（尚未同步到服务器）。确定清空界面上的所有消息？")
      )
        return;
      startNewChat(false);
    }
  ...
  async function sendMessage(content: string) {
    ...
    if (workCtx) {
      aiContent +=
        `\n\n【关联工单数据】\n` +
        `工单编号：#${workCtx.id}\n` +
        `地下病害类型：${workCtx.type} | 危险等级：${sevLabel(workCtx.severity)} | 处理状态：${statusLabel(workCtx.status)}\n` +
        `路段：${workCtx.road}\n` +
    ...
    if (USE_MOCK) {
      try {
        ...
        const { stripped, hint } = stripUiContextTag(reply);
        applyUiContextFromHint(hint);
    ...
    try {
      ...
      time: formatLocaleTimeHm(),
          ...
          sawDone = true;
          const { stripped, hint } = stripUiContextTag(acc);
          applyUiContextFromHint(hint);
          patchLoading({
            content: stripped,
            deepthought: dt,
  ...
  function mockReply(input: string): { reply: string; card?: ActionCard } {
    ...
    if (
      ...
      return {
        reply:
          "已结合地下病害工单上下文完成初判：\n\n" +
          "- 风险判定：建议按 **空洞 > 脱空 > 疏松** 的优先级组织复核\n" +
          "- 处置建议：先排除管线/地铁施工影响，再安排分段复测与注浆加固\n" +
          "- 闭环动作：补充检测剖面、更新工单进度、设置复检时间窗",
        card: {
          type: "workorder",
          title: "地下病害工单分析",
          subtitle: "空洞 / 脱空 / 疏松 风险建议",
        },
      };
    }
    if (
      ...
      return {
        reply:
          "当前共有 **23张**活跃工单，其中待处理 8 张、处理中 10 张、待验收 5 张。",
        card: {
          type: "workorder",
          title: "病害工单看板",
          subtitle: "23张活跃工单",
    ...
    if (
      ...
      return {
        reply:
          "**计划生成**模块暂不可用。可先通过 **病害工单** 查看待巡检与排期相关任务；开放后将支持一键生成季度/月度巡检计划。",
      };
    }
    if (
      input.includes("风险") ||
    ...
    if (
      ...
      const district = input.includes("西湖区")
        ? "西湖区"
        : input.includes("上城区")
          ? "上城区"
          : "滨江区";
  ...
  async function submitLike(
    ...
    const thread = activeConversationKey.value
      ? threadPartFromKey(activeConversationKey.value)
      : activeThreadShort.value;
